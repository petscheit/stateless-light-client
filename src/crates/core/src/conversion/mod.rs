...
use crate::fetcher::{recursive_epoch_input::SignerData, sync_committee_input::SyncCommitteeData};
use crate::fetcher::{
    recursive_epoch_input::{
        EpochUpdate, G1Point, G2Point, RecursiveEpochInputs, RecursiveEpochOutput,
        RecursiveEpochUpdate,
    },
    sync_committee_input::CommitteeUpdateData,
};
...
impl From<CommitteeUpdateData> for CommitteeUpdateDataCairo {
    fn from(val: CommitteeUpdateData) -> Self {
        ...
        CommitteeUpdateDataCairo {
            slot: Felt(Felt252::from(val.slot)),
            path: branch,
            next_aggregate_sync_committee: UInt384(BigUint::from_bytes_be(
                val.next_aggregate_sync_committee.as_slice(),
            )),
...
impl From<SignerData> for SignerDataCairo {
